...
-(void) moveItemsRightBy:(float) offset
{
	...
	CCMenuItem* lastItem = [children_ objectAtIndex:[children_ count] - 1];
	if([lastItem position].x + [self position].x - [lastItem contentSize].width / 2.0 > 320)//480
	{
		[[SimpleAudioEngine sharedEngine] playEffect:@"dragteam.caf"];
		[lastItem retain];
...
-(void) updateAnimation
{
	...
	for(CCMenuItem<CCRGBAProtocol>* item in children_)
	{
		float distance = fabsf([item position].x - 160.0 + [self position].x); //240
        
		if(distance > 160.0) //160
			distance = 160.0;
		...
		[item setPosition:ccp([item position].x, yOffset - (lowerBound - [item contentSize].height * ratio / 2.0))];
